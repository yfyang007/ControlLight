...
def get_watchdog_schedule(
    ...
    if shift:
        mu = ((max_shift - base_shift) / (4096 - 256)) * image_seq_len + (
            base_shift - ((max_shift - base_shift) / (4096 - 256)) * 256
        )
        timesteps = math.exp(mu) / (math.exp(mu) + (1 / timesteps - 1) ** 1.0)
    return timesteps.tolist()


def _watchdog_cap_pixels(img: PIL.Image.Image, limit_pixels: int = 1024**2) -> PIL.Image.Image:
    w, h = img.size
    pixel_count = w * h
    if pixel_count <= limit_pixels:
        return img
    scale = math.sqrt(limit_pixels / pixel_count)
    new_w = int(w * scale)
    new_h = int(h * scale)
    return img.resize((new_w, new_h), PIL.Image.Resampling.LANCZOS)


def _watchdog_center_crop_multiple(img: PIL.Image.Image, multiple: int = 16) -> PIL.Image.Image:
    w, h = img.size
...
def _watchdog_prepare_condition_tensor(img: PIL.Image.Image, ensure_multiple: int = 16) -> torch.Tensor:
    img = img.convert("RGB")
    img = _watchdog_cap_pixels(img, 1024**2)
    img = _watchdog_center_crop_multiple(img, multiple=ensure_multiple)
...
class Flux2KleinPipeline(DiffusionPipeline, Flux2LoraLoaderMixin):
    ...
    def __call__(
        ...
        if image is not None:
            ...
            for img in image:
                if watchdog_compat:
                    img = _watchdog_prepare_condition_tensor(img, ensure_multiple=self.vae_scale_factor * 2)
                    condition_images.append(img)
                    height = height or int(img.shape[-2])
                    width = width or int(img.shape[-1])
                else:
        ...
        if watchdog_compat:
            timesteps = get_watchdog_schedule(num_inference_steps, latents.shape[1])
                ...
                for i, (t_curr, t_prev) in enumerate(zip(timesteps[:-1], timesteps[1:])):
                    ...
                    with self.transformer.cache_context("cond"):
                        noise_pred = self.transformer(
                            ...
                            timestep=timestep,
                            guidance=guidance_vec.to(self.transformer.dtype),
                    ...
                    latents = latents + (t_prev - t_curr) * noise_pred.to(latents.dtype)
