    ...
    for sub_dir in extension_folders:
        ...
        for (_, name, _) in pkgutil.iter_modules([extensions_dir]):
            try:
                ...
                print(f"Failed to import the {name} extension. Error: {str(e)}")
...
def get_requested_extensions_process_dict(requested_uids) -> dict:
    requested = {uid for uid in requested_uids if uid}
    if not requested:
        return get_all_extensions_process_dict()

    all_extensions = get_all_extensions()
    process_dict = {}
    for extension in all_extensions:
        if extension.uid not in requested:
            continue
        process_dict[extension.uid] = extension.get_process()

    missing = requested - set(process_dict)
    if missing:
        raise ValueError(f"Unknown extension process type(s): {sorted(missing)}")
    return process_dict
