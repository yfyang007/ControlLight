...
class CaptionProcessingDTOMixin:
    ...
    def load_caption(self: 'FileItemDTO', caption_dict: Union[dict, None]=None):
        if self.raw_caption is not None:
            ...
            if 'network_weight' in caption_dict[self.path]:
                self.network_weight = float(caption_dict[self.path]['network_weight'])
            if 'extra_values' in caption_dict[self.path]:
                self.extra_values = caption_dict[self.path]['extra_values']
        else:
            # see if prompt file exists
            ...
            if os.path.exists(prompt_path):
                with open(prompt_path, 'r', encoding='utf-8') as f:
                    ...
                    if prompt_path.endswith('.json'):
                        ...
                        if 'network_weight' in prompt_json:
                            self.network_weight = float(prompt_json['network_weight'])
...
class ControlFileItemDTOMixin:
    ...
    def load_control_image(self: 'FileItemDTO'):
        ...
        if len(control_tensors) == 0:
            ...
            # Keep raw control images as a per-item list even when there is only
            # one control image. Raw control images can have different spatial
            # sizes across items in a batch, so DataLoaderBatchDTO must not
            # concatenate them into a single dense tensor.
...
class LatentCachingMixin:
    ...
    def _get_latent_cache_done_path(self: 'AiToolkitDataset'):
        # Use the same image-local _latent_cache directory as get_latent_path().
        # The cache path hash already depends on crop/resize/latent metadata; this
        # sentinel only tells non-main ranks when rank 0 has finished the long
        # cache pass so they do not wait inside torch.distributed for >10 minutes.
        if len(self.file_list) == 0:
            latent_dir = os.path.join(str(self.dataset_path), '_latent_cache')
        else:
            first_latent_path = self.file_list[0].get_latent_path(recalculate=True)
            latent_dir = os.path.dirname(first_latent_path)

        cache_key = {
            'dataset_path': str(self.dataset_path),
            'num_files': len(self.file_list),
            'first_path': str(self.file_list[0].path) if len(self.file_list) > 0 else None,
            'last_path': str(self.file_list[-1].path) if len(self.file_list) > 0 else None,
            'first_latent_path': first_latent_path if len(self.file_list) > 0 else None,
        }
        key_bytes = json.dumps(cache_key, sort_keys=True).encode('utf-8')
        cache_hash = hashlib.md5(key_bytes).hexdigest()[:16]
        return os.path.join(latent_dir, f'.aitoolkit_latents_done_{cache_hash}.json')

    def _mark_disk_latents_cached(self: 'AiToolkitDataset'):
    ...
    def _write_latent_cache_done(self: 'AiToolkitDataset', done_path: str):
    ...
    def _wait_for_latent_cache_done(self: 'AiToolkitDataset', done_path: str):
        timeout = float(os.environ.get('AI_TOOLKIT_CACHE_WAIT_TIMEOUT_SEC', '0') or 0)
        start_time = time.time()
        print_acc(f"Waiting for rank 0 latent cache sentinel: {done_path}")
        while not os.path.exists(done_path):
            if timeout > 0 and time.time() - start_time > timeout:
                raise TimeoutError(f"Timed out waiting for latent cache sentinel: {done_path}")
            time.sleep(5)
    ...
    def cache_latents_all_latents(self: 'AiToolkitDataset'):
        ...
        # For multi-GPU cache-to-disk, do not use accelerator.main_process_first().
        # That context uses a torch.distributed barrier; FLUX latent caching can
        # take longer than the default 600s c10d timeout, so non-main ranks die
        # while rank 0 is still writing valid cache files. Use a filesystem
        # sentinel instead and only rank 0 performs the expensive VAE encoding.
        use_filesystem_wait = (
            accelerator.num_processes > 1
            and self.is_caching_latents_to_disk
            and not self.is_caching_latents_to_memory
        )
        if use_filesystem_wait:
            done_path = self._get_latent_cache_done_path()
            if accelerator.is_main_process:
                self._cache_latents_all_latents_impl()
                self._write_latent_cache_done(done_path)
            else:
                self._wait_for_latent_cache_done(done_path)
                self._mark_disk_latents_cached()
            return

        with accelerator.main_process_first():
            self._cache_latents_all_latents_impl()

    def _cache_latents_all_latents_impl(self: 'AiToolkitDataset'):
