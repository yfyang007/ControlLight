...
class BaseSDTrainProcess(BaseTrainProcess):
    ...
    def __init__(self, process_id: int, job, config: OrderedDict, custom_pipeline=None):
        ...
        self.distributed_barrier_run_id = os.environ.get(
            "AI_TOOLKIT_RUN_ID",
            os.environ.get("TORCHELASTIC_RUN_ID", "default"),
        )
    ...
    def prepare_accelerator(self):
        ...
        # Dataloaders are sharded in toolkit.data_loader with DistributedSampler.
        # Do not pass bucketed ai-toolkit dataloaders to accelerator.prepare():
        # they use batch_size=None because the dataset returns complete bucket
        # batches, and Accelerate's BatchSamplerShard expects batch_sampler to be
        # iterable, which fails for this mode.
    ...
    def distributed_filesystem_barrier(self, name: str, step: Optional[int] = None):
        """
        Filesystem barrier for long or rank-asymmetric I/O sections.

        accelerator.wait_for_everyone() uses the distributed backend. For this
        FLUX2 LoRA job, save/sample points can arrive after very long variable
        resolution steps, and a NCCL barrier can become the watchdog failure
        itself. This barrier keeps ranks aligned using local files instead, so
        save/sample synchronization does not create long-lived NCCL work.
        """
        ...
        step_part = "none" if step is None else str(step).zfill(9)
        barrier_dir = os.path.join(
            self.save_root,
            ".distributed_barriers",
            str(self.distributed_barrier_run_id),
            f"{step_part}_{name}",
        )
        os.makedirs(barrier_dir, exist_ok=True)

        marker_path = os.path.join(barrier_dir, f"rank_{rank}")
        tmp_marker_path = f"{marker_path}.tmp.{os.getpid()}"
        with open(tmp_marker_path, "w", encoding="utf-8") as f:
            f.write(f"{time.time()}\n")
        os.replace(tmp_marker_path, marker_path)

        timeout = float(os.environ.get("AI_TOOLKIT_FS_BARRIER_TIMEOUT_SEC", "7200") or 0)
        start_time = time.time()
        while True:
            present = 0
            for idx in range(num_processes):
                if os.path.exists(os.path.join(barrier_dir, f"rank_{idx}")):
                    present += 1
            if present >= num_processes:
                return
            if timeout > 0 and time.time() - start_time > timeout:
                raise TimeoutError(
                    f"Timed out waiting for filesystem barrier {barrier_dir}: "
                    f"{present}/{num_processes} ranks arrived"
                )
            time.sleep(1)
    ...
    def run(self):
        ...
        print_acc(
            f"Using model class {ModelClass.__name__} for arch={self.model_config.arch}"
        )
        if (
            self.model_config.arch is not None
            and str(self.model_config.arch).startswith("flux2")
            and ModelClass is StableDiffusion
        ):
            raise ValueError(
                f"arch={self.model_config.arch} resolved to StableDiffusion. "
                "This would try to load a FLUX.2 checkpoint with StableDiffusionPipeline. "
                "Check that extensions_built_in.diffusion_models.flux2 imports correctly."
            )
        ...
        for step in range(start_step_num, self.train_config.steps):
            ...
            with torch.no_grad():
                ...
                is_reg_step = False
                is_save_step = False
                if self.save_config.save_every:
                    # When resuming from a checkpoint (start_step > 0), schedule
                    # periodic saves relative to the resumed step instead of the
                    # absolute global step.  Example: start_step=500 and
                    # save_every=150 now saves at 650, 800, 950, ... rather than
                    # 600, 750, 900, ... .  This makes checkpoint-sidecar
                    # visualization cadence match "every N steps" after resume.
                    save_step_offset = self.step_num - self.start_step
                    if self.start_step and save_step_offset > 0:
                        is_save_step = save_step_offset % self.save_config.save_every == 0
                    else:
                        is_save_step = self.step_num % self.save_config.save_every == 0
            ...
            with torch.no_grad():
                ...
                if self.step_num != self.start_step:
                    if is_sample_step or is_save_step:
                        self.distributed_filesystem_barrier("pre_io", self.step_num)
                    ...
                    if is_sample_step or is_save_step:
                        self.distributed_filesystem_barrier("post_io", self.step_num)
        ...
        ##  END TRAIN LOOP
        ###################################################################
        self.distributed_filesystem_barrier("end_train", self.step_num)
        if self.progress_bar is not None:
