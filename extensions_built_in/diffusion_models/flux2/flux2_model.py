...
class Flux2Model(BaseModel):
    ...
    def generate_single_image(
        ...
        # Optional one-off velocity-field experiment hooks. These are inert unless
        # the corresponding environment variables are set. They deliberately live
        # on the pipeline instance so the exact production generation path is used.
        trace_dir = os.environ.get("AITK_FLUX2_VELOCITY_TRACE_DIR", None)
        if trace_dir:
            pipeline._aitk_velocity_trace_dir = trace_dir
            pipeline._aitk_velocity_trace_name = os.path.splitext(
                os.path.basename(gen_config.output_path or "sample")
            )[0]
            pipeline._aitk_velocity_trace_alpha = float(gen_config.network_multiplier)
            pipeline._aitk_velocity_trace_steps = os.environ.get(
                "AITK_FLUX2_VELOCITY_TRACE_STEPS", "0"
            )
        else:
            pipeline._aitk_velocity_trace_dir = None

        direction_mode = None
        if os.environ.get("AITK_FLUX2_PIECEWISE_DIRECTION_MODE", "0") == "1":
            direction_mode = "piecewise"
        elif os.environ.get("AITK_FLUX2_LS_DIRECTION_MODE", "0") == "1":
            direction_mode = "ls"
        elif os.environ.get("AITK_FLUX2_LINEAR_DIRECTION_MODE", "0") == "1":
            direction_mode = "endpoint"

        if direction_mode is not None:
            pipeline._aitk_direction_mode = direction_mode
            pipeline._aitk_linear_direction_alpha = float(gen_config.network_multiplier)
            pipeline._aitk_linear_direction_hi = float(
                os.environ.get("AITK_FLUX2_LINEAR_DIRECTION_HI", "0.9")
            )
            pipeline._aitk_linear_direction_network = self.network
            pipeline._aitk_piecewise_direction_anchors = os.environ.get(
                "AITK_FLUX2_PIECEWISE_DIRECTION_ANCHORS",
                "0,0.18,0.27,0.36,0.54,0.63,0.81,0.9",
            )
            pipeline._aitk_ls_direction_probes = os.environ.get(
                "AITK_FLUX2_LS_DIRECTION_PROBES",
                "0.18,0.27,0.36,0.54,0.63,0.81",
            )
        else:
            pipeline._aitk_direction_mode = None
            pipeline._aitk_linear_direction_alpha = None
            pipeline._aitk_linear_direction_network = None
        ...
        **kwargs,
        ...
        with torch.no_grad():
            ...
            if batch_control_tensor_list is not None:
                ...
                target_control_height = None
                target_control_width = None
                if self.model_config.model_kwargs.get("match_target_res", False):
                    # Use the exact current target pixel size for control refs.
                    # Keeping each raw control image's original aspect ratio can
                    # produce different encoded token counts inside one bucketed
                    # batch, which breaks bs>1 concatenation. Exact target resize
                    # also gives the intended spatially aligned edit signal.
                    target_control_height = int(height * self.pipeline.vae_scale_factor)
                    target_control_width = int(width * self.pipeline.vae_scale_factor)
                    control_image_res = target_control_height * target_control_width
                ...
                for control_tensor_list in batch_control_tensor_list:
                    ...
                    for control_img in control_tensor_list:
                        ...
                        # resize to exact target resolution for batch-compatible,
                        # spatially aligned control conditioning
                        if self.model_config.model_kwargs.get(
                            "match_target_res", False
                        ):
                            control_img = F.interpolate(
                                control_img,
                                size=(target_control_height, target_control_width),
                                mode="bilinear",
                                align_corners=False,
                            )
    ...
    def decode_latents(self, latents: torch.Tensor, device=None, dtype=None):
        ...
        # Move VAE to the execution device if it is currently parked on CPU.
        if self.vae.device == torch.device("cpu"):
            self.vae.to(device)

        # The custom FLUX2 Klein AutoEncoder expects its input latents to match
        # the VAE parameter dtype exactly. In multi-GPU training the structural
        # consistency branch can hand us float32 latents while the VAE weights
        # stay in bf16, which causes conv bias dtype mismatches during decode.
